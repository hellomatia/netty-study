...
### select
- select는 지정된 파일 디스크립터 집합을 감시하며, 파일 디스크립터의 최대값에 제한이 있어 최대 약 1024개까지 모니터링할 수 있다.
- 이 방식은 매번 모든 파일 디스크립터를 스캔하여 상태를 확인하기 때문에, 파일 디스크립터 수가 많아질수록 성능이 저하된다.
- 또한 매 호출 시마다 파일 디스크립터 집합을 다시 설정해야 해서 리소스 낭비가 발생할 수 있다.

### poll
- poll은 select와 유사하게 여러 파일 디스크립터의 상태를 감지한다.
- 파일 디스크립터 집합을 연결 리스트 형태로 관리해, 파일 디스크립터의 최대 개수에 제한이 없다는 장점이 있다.
- 하지만 poll 역시 모든 파일 디스크립터를 매번 확인하는 방식이라, 파일 디스크립터가 많을수록 비효율적이다.

### epoll
epoll은 리눅스에서 다수의 파일 디스크립터(file descriptor)를 효율적으로 관리하기 위한 I/O 이벤트 감지 메커니즘이다. 
고성능 서버에서 사용되며, 특히 비동기 방식의 네트워크 애플리케이션에서 유용하다.

일반적인 select나 poll과 달리, epoll은 등록된 파일 디스크립터의 상태 변화를 계속해서 감시할 필요 없이 이벤트가 발생할 때만 반환한다.
이로 인해 불필요한 CPU 자원 소모가 줄어들고 성능이 크게 향상된다.
